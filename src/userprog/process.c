...
static inline void parse_elf_name(const char* src,char* dst) {                                       //tach lay elf name (ten file name khi loai bo argument)
  size_t i;
  ...
  strlcpy (dst, src, i+1);                                                                           // cat bo phan elf name ra 

}

static void construct_argument_stack(const char* cmdline,uint32_t** esp)                            // day phan argument con lai vao stack
{                                                                                                     
  char* sp=*esp;                                                                                    //stack pointer =  esp ( gia tri ma thanh ghi vua luu khi ngat)
  ...
  while(i<len){
    ...
    start_pos=i;                                                                                    // diem bat dau stack
    while(cmdline[i]!=' '&&cmdline[i]!=NULL){
      i++;
    }
    size_t size=i-start_pos+1;
    argv_temp[j]=malloc(size*sizeof(char));                                                         // dua cac argv vao stack theo thu tu 
  ...
  size_t alignment=(uint32_t)sp%4;                                                                  // can chinh con tro xuong boi so cua 4
  sp-=alignment;
  ASSERT((int)sp%4==0);
  ...
  *esp=sp;
  ...
  for(i=0;i<argc;i++){                                                                        //giai phong cac bien temp
    free(argv_temp[i]);
  }
  free(argv_address);
  free(argv_temp);
...
process_execute (const char *file_name) 
{
  ...
  created = thread_create (ELF_NAME, PRI_DEFAULT, start_process, fn_copy);                                // tao luong voi ten file moi (elf name)
...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp); /* Hàm load sẽ dùng để tìm file có thể tiến hành được, sử dụng tên file và tải file từ disk vào bộ nhớ .  3 tham số vào hàm load : tên chương trình , Function entry point, stack top pointer (user stack): con trỏ đầu ngăn xếp người dùng
*/
